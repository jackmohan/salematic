...
exports.hourly_reports = function(res, hourlyArray, from_date, to_date, fuelInd, vehicles, groups, group_id, vehicle, login_name, pageName)
{
    ...
    {
        // console.log(hourlyArray[i]['vht_device_startdatetime']);
        stTime = moment(hourlyArray[i]['vht_device_startdatetime']);
        stTimeFormat = stTime.format('hh A');
        stDateFormat = stTime.format('ddd D MMM YYYY');
        endTime = moment(hourlyArray[i]['vht_device_enddatetime']);
        endTimeFormat = endTime.format('hh A');
        endDateFormat = endTime.format('ddd D MMM YYYY');
        distanceMilege = parseFloat(hourlyArray[i]['vht_end_mileage']) - parseFloat(hourlyArray[i]['vht_start_mileage']);
        if(distanceMilege != 0)
        {
        ...
        if(chkNull(hourlyArray[i]['vht_fuel_start_capacity']) != 0 && chkNull(hourlyArray[i]['vht_fuel_end_capacity']) != 0)
        {
            console.log(parseFloat(hourlyArray[i]['vht_fuel_start_capacity'])  + ' --- ' + parseFloat(hourlyArray[i]['vht_fuel_end_capacity']) + ' --- ' + parseFloat(chkNull(hourlyArray[i]['vht_refill_ltrs'])));
            fuelLevel = parseFloat(hourlyArray[i]['vht_fuel_start_capacity']) - parseFloat(hourlyArray[i]['vht_fuel_end_capacity']) + parseFloat(chkNull(hourlyArray[i]['vht_refill_ltrs']));
            if(fuelLevel > 0)
            {
        ...
        console.log("I VAL : " + i);
        hourlyResult.push({'stTimeFormat': stTimeFormat, 'stDateFormat': stDateFormat, 'endDateFormat' : endDateFormat, 'endTimeFormat' : endTimeFormat, 'distanceMilege' : distanceMilege,'fuelLevel': fuelLevel, 'mileage' : mileage, 'stLocation' : '', 'endLocation' : '', 'fmDtFormat': moment(hourlyArray[i]['vht_device_startdatetime']).format('DD/MM/YYYY hh:00 A'), 'toDtFormat': moment(hourlyArray[i]['vht_device_enddatetime']).format('DD/MM/YYYY hh:00 A')}); 
        // console.log(hourlyResult);
        // console.log(hourlyArray.length + ' == ' + hourlyResult.length);
        if(hourlyArray.length ==  hourlyResult.length)
        {
            res.render(pageName,{ title: 'Hourly Report', 'from_date': from_date, 'to_date': to_date, fuelInd : fuelInd, 'vehicles' : vehicles, 'groups': groups, 'group_id' : group_id, 'vehicle': vehicle, 'trip_summary': hourlyResult,  'login_name' : login_name});
        }
    }
